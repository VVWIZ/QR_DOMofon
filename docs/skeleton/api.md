...
| GET | `/api/v1/admin/schedule-points` | admin | Калитки/шлагбаумы своей УК + их окна авто-открытия | 200 | 401, 403 |
| POST | `/api/v1/admin/access-points/{public_id}/schedules` | admin | Добавить окно авто-открытия | 201 | 400, 401, 403 |
| DELETE | `/api/v1/admin/schedules/{id}` | admin | Удалить окно | 200 | 400, 401, 403 |
...
## Авто-открытие по расписанию (инкремент E, «время работы» точек)

Калитки/шлагбаумы могут авто-открываться в заданные часы (напр. въезд открыт днём). Окно = день недели + `[opens, closes)` в таймзоне точки. Только `gate`/`barrier`, скоуп по `mc`.
- `GET /admin/schedule-points` → точки + их окна: `{points:[{public_id,label,type,schedules:[{id,dow,opens,closes,timezone,is_active}]}]}`. `dow`: 0=вс … 6=сб.
- `POST /admin/access-points/{public_id}/schedules` — тело `{dow, opens:"HH:MM", closes:"HH:MM", timezone:"Asia/Almaty"}`; `closes>opens`, валидная IANA-tz, точка — gate/barrier своей УК.
- `DELETE /admin/schedules/{id}` — удалить окно (скоуп по mc).

**Планировщик (fail-secure).** In-process reconciler (тик 15с, конфиг `SCHEDULER_TICK/SCHEDULER_LEASE`) под `pg_try_advisory_lock` (защита от двойного инстанса). Каждый тик вычисляет активные окна (в tz точки, с учётом DST/сдвига дня) и точкам «должно быть открыто сейчас» **переиздаёт короткую аренду** `open_relay` (уникальный `request_id` → устройство переактуирует; `duration = SCHEDULER_LEASE > tick`). «Держим открытым» = продлеваем аренду каждый тик; команду «закрыть» НЕ шлём. **Отказ планировщика/сервера → аренда истекает, реле закрывается само** (fail-secure — симметрично политике fail-open). Аудит `scheduled_open_started`/`scheduled_open_ended` — на переходах окна (пульсы не аудируются); `schedule_created`/`schedule_deleted` — на CRUD.
