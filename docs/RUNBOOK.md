...
Создать владельца (выбор дом→подъезд→квартира из каталога + ФИО + чекбоксы калиток/шлагбаумов →
композитный инвайт одной ссылкой), выдать доступ на калитку/шлагбаум, список жильцов. Инвайт-ссылка
возвращается прямо в консоли — админ **копирует и сам пересылает** её адресату (SMS/мессенджер);
это целевой канал доставки, а не заглушка.
