...
> **TOTP в dev отключён.** При `AUTH_DEV_MODE=true` проверка 2FA для входа админа/sysadmin
> **пропускается** — поле «Код 2FA» можно оставить пустым (пароль остаётся обязательным). В проде
> (`AUTH_DEV_MODE=false`) TOTP обязателен как прежде.
...
квартиры с их чекбоксами. Кнопка «Калитки/Шлагбаумы» открывает **расписания авто-открытия** (время
работы): день недели + окно + таймзона на точку.

**Планировщик авто-открытия** стартует вместе с backend (лог `scheduler_started`). Период/аренда —
`SCHEDULER_TICK` (дефолт 15с) / `SCHEDULER_LEASE` (40с); для быстрой демо-проверки можно уменьшить:
`SCHEDULER_TICK=8s SCHEDULER_LEASE=20s go run ./cmd/server`. В окне точка удерживается открытой
арендой; отказ сервера → закрывается сама (fail-secure). Для наблюдения нужен запущенный эмулятор точки.
