...
Скоуп skeleton: **одна захардкоженная квартира**, без RBAC и аутентификации. Вне скоупа (анти-скоуп этапа): RBAC, гости/курьеры, гео-проверка, фотофиксация, push, OTP, мобильное приложение, реальное железо, hash-chain аудита, OTA, TLS для MQTT, rate-limit, multi-tenancy enforcement, админки, 2FA.
...
| `backend/` | Go модульный монолит: `cmd/server` (API :8080), `cmd/qrgen` (генератор QR-URL), `internal/{platform,qr,property,calls,access,devices,audit}`, `migrations/` |
| `web/visitor/` | React/TS SPA (Vite): роут `/v` — посетитель, `/resident` — жилец |
